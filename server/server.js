...
const FRONTEND_URL=(process.env.FRONTEND_URL||'https://masudjr-official-online.netlify.app').replace(/\/$/,'');

app.use(cors({
  origin:(origin,cb)=>{
    if(!origin||origin===FRONTEND_URL||origin.endsWith('.netlify.app'))
      return cb(null,true);
    cb(null,false);
  },
  credentials:false
...
q(`CREATE TABLE IF NOT EXISTS admin_sessions(
  session TEXT PRIMARY KEY,
...
q(`CREATE TABLE IF NOT EXISTS task_attempts(
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  user_id INTEGER NOT NULL,
...
q(`CREATE TABLE IF NOT EXISTS task_completions(
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  user_id INTEGER NOT NULL,
  task_id INTEGER NOT NULL,
  reward INTEGER NOT NULL,
...
q(`CREATE TABLE IF NOT EXISTS notices(
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  title TEXT NOT NULL,
...
q(`CREATE TABLE IF NOT EXISTS fee_payments(
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  user_id INTEGER NOT NULL,
...
function userFromReq(req){
  ...
  return q(`
    ...
    WHERE s.session=? AND s.expires_at>?
  `).get(s,now())||null;
}

function adminFromReq(req){
  const s=req.header('x-admin-session');
  if(!s)return null;
...
if(q('SELECT COUNT(*) c FROM tasks').get().c===0){
  const ins=q(`
    INSERT INTO tasks(
      title,type,description,url,reward,created_at
    )
    VALUES(?,?,?,?,?,?)
  `);
...
if(q('SELECT COUNT(*) c FROM notices').get().c===0){
  q(`
    INSERT INTO notices(title,message,created_at)
    VALUES(?,?,?)
  `).run(
    'স্বাগতম',
    'Masud JR Official-এ স্বাগতম। নতুন task ও গুরুত্বপূর্ণ ঘোষণা এখানে পাবেন।',
    now()
  );
}
...
app.get('/api/register',(req,res)=>{
  res.status(405).json({
    ok:false,
    message:'Registration endpoint is POST only.'
  });
});
...
app.post(['/api/register','/api/register/'],(req,res)=>{
  ...
  const u=q(`
    SELECT id,name,balance,blocked,created_at
    FROM users
    WHERE id=?
  ...
  const tasks=rows.map(r=>{
    ...
    if(r.last_completed){
      available_at=new Date(
        new Date(r.last_completed).getTime()
        +5*3600e3
      ).toISOString();
    }
    ...
    return {
      ...
      available_at:
        available_at&&
        new Date(available_at)>new Date()
          ?available_at
          :null
    };
  });
...
app.post('/api/tasks/:id/start',(req,res)=>{
  ...
  q(`
    INSERT INTO task_attempts(
      user_id,task_id,started_at,last_heartbeat,status
    )
    VALUES(?,?,?,?,?)
  `).run(
    u.id,
...
app.post('/api/tasks/:id/complete',(req,res)=>{
  ...
  const tx=db.transaction(()=>{
    ...
    if(!attempt){
      throw new Error(
        'Active Task session পাওয়া যায়নি।'
      );
    }
...
app.get('/api/fee-status',(req,res)=>{
  ...
  res.json({
    required:!f||f.status!=='approved',
    status:f?.status||'required'
  });
});
...
app.post('/api/fee-payments',(req,res)=>{
  ...
  q(`
    INSERT INTO fee_payments(
      user_id,transaction_id,status,created_at
    )
    VALUES(?,?,?,?)
  `).run(
    u.id,
...
app.post('/api/withdrawals',(req,res)=>{
  ...
  if(!['bKash','Nagad','Rocket'].includes(method)){
    return res.status(400).json({
      message:'Payment method ভুল।'
    });
  }

  if(!/^01\d{9}$/.test(num)){
    return res.status(400).json({
      message:'সঠিক ১১ সংখ্যার account number দিন।'
    });
  }

  if(!Number.isInteger(amount)||amount<200){
    return res.status(400).json({
      message:'Minimum withdrawal ৳200।'
    });
  }
...
app.post('/api/admin/login',(req,res)=>{
  ...
  q(`
    INSERT INTO admin_sessions(
      session,admin_id,created_at,expires_at
    )
    VALUES(?,?,?,?)
  `).run(
    s,
...
app.post('/api/admin/users/:id/block',adminOnly,(req,res)=>{
  q(`
    UPDATE users
    SET blocked=?
    WHERE id=?
...
app.post('/api/admin/tasks',adminOnly,(req,res)=>{
  ...
  if(
    ...
    return res.status(400).json({
      message:'Title ও reward দিন'
    });
  }

  const id=q(`
    INSERT INTO tasks(
      title,type,description,url,reward,enabled,created_at
    )
    VALUES(?,?,?,?,?,?,?)
  `).run(
    title,
...
app.post('/api/admin/tasks/:id/toggle',adminOnly,(req,res)=>{
  q(`
    UPDATE tasks
    SET enabled=?
    WHERE id=?
...
app.post('/api/admin/tasks/:id/update',adminOnly,(req,res)=>{
  const {
    title,
    type,
    description,
...
app.get('/api/admin/fees',adminOnly,(req,res)=>{
  res.json({
    fees:q(`
      SELECT f.*,u.name
      FROM fee_payments f
...
app.post('/api/admin/fees/:id/status',adminOnly,(req,res)=>{
  const status=String(
    req.body.status||''
  );
...
app.get('/api/admin/notices',adminOnly,(req,res)=>{
  res.json({
    notices:q(`
      SELECT *
      FROM notices
...
app.post('/api/admin/notices',adminOnly,(req,res)=>{
  const {
    title,
    message,
    enabled=true
  ...
  const id=q(`
    INSERT INTO notices(
      title,message,enabled,created_at
    )
    VALUES(?,?,?,?)
  `).run(
    title,
...
app.post('/api/admin/notices/:id/toggle',adminOnly,(req,res)=>{
  q(`
    UPDATE notices
    SET enabled=?
    WHERE id=?
